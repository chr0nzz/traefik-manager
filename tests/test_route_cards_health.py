...
def test_the_unreachable_flag_reaches_the_card_and_the_routes_filter():
    ...
    assert "tn('route unreachable', 'routes unreachable', m.http.t.down)" in dash, 'the verdict line counts routes, so it must say routes'
...
def test_the_degraded_count_reaches_the_card_the_verdict_and_the_entry_points():
    ...
    assert "tn('route degraded', 'routes degraded', m.http.t.degraded)" in dash, 'the verdict line needs a degraded item that counts routes'
