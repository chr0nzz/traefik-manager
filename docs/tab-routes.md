...
| Rule Mode | *(HTTP)* **Simple** (default) builds a `Host()` rule from the Subdomain + Domain chips. **Advanced rule** takes any valid Traefik rule (`PathPrefix`, `HostRegexp`, compound rules with `&&` / `\|\|`). A route opens in Simple only when the simple form can write its rule back exactly; any other rule opens in Advanced. The two are kept apart: the Advanced field always holds the route's rule as it is, and only the mode selected when you save is written, so switching to Simple and back never changes the rule. Simple mode warns when the rule does not fit it, since saving there replaces it. The rule field grows to show a long rule in full and can be resized; Enter still saves the route. |
