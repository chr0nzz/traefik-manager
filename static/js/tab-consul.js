let _allConsulRoutes = [];
let _consulFilter    = 'all';

async function refreshConsulTab() {
    const container = document.getElementById('consulContent');
    container.innerHTML = `<div class="text-center py-16" style="color:var(--muted)"><i class="ph-light ph-spinner-gap text-4xl block mb-3 animate-spin opacity-40"></i><p>${th('Loading Consul KV routes…')}</p></div>`;
    ...
    try {
        ...
        _allConsulRoutes = all
            .filter(r => getProvider(r) === 'consul')
        ...
        if (_allConsulRoutes.length === 0) {
            container.innerHTML = `<div class="text-center py-16 rounded-xl" style="color:var(--muted);border:1px solid var(--border)"><i class="ph-light ph-database text-5xl block mb-3 opacity-30"></i><p class="font-medium">${th('No Consul KV routes found')}</p><p class="text-xs mt-1">${th('Routes stored in Consul KV will appear here')}</p></div>`;
            return;
        }
...
function filterConsul(f) {
    if (f) {
        _consulFilter = f;
        ['all', 'http', 'tcp', 'udp'].forEach(k => {
            const btn = document.getElementById('ck-' + k);
...
function renderConsulRoutes() {
    const search = (document.getElementById('consulSearch')?.value || '').toLowerCase();
    const items  = _allConsulRoutes.filter(r => {
        if (_consulFilter !== 'all' && r._proto.toLowerCase() !== _consulFilter) return false;
    ...
    if (items.length === 0) {
        document.getElementById('consulContent').innerHTML =
    ...
    const cards = items.map(r => {
        const globalIdx = _allConsulRoutes.indexOf(r);
        return renderProviderCard(r, { onDetailClick: `openConsulRouteDetail(${globalIdx})` });
    }).join('');
...
async function openConsulRouteDetail(idx) {
    const r = _allConsulRoutes[idx];
    if (!r) return;

    document.getElementById('detailEditBtn').style.display = 'none';

    const badge = `<span class="d-flat d-off ml-2"><i class="ph-bold ph-database"></i> consul kv</span>`;
