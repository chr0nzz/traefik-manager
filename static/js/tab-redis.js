...
async function refreshRedisTab() {
    ...
    container.innerHTML = `<div class="text-center py-16" style="color:var(--muted)"><i class="ph-light ph-spinner-gap text-4xl block mb-3 animate-spin opacity-40"></i><p>${th('Loading Redis routes…')}</p></div>`;
    ...
    try {
        ...
        if (_allRedisRoutes.length === 0) {
            container.innerHTML = `<div class="text-center py-16 rounded-xl" style="color:var(--muted);border:1px solid var(--border)"><i class="ph-light ph-database text-5xl block mb-3 opacity-30"></i><p class="font-medium">${th('No Redis routes found')}</p><p class="text-xs mt-1">${th('Routes stored in Redis will appear here')}</p></div>`;
            return;
        }
