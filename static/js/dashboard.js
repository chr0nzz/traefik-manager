...
function _sdRender(model) {
    ...
    if (verdEl) {
        const items = [];
        if (m.http.t.disabled)       items.push(_sdExc('d-bad',  'ph-fill ph-x-circle',            m.http.t.disabled,       t('routers disabled'),     'tab=services;proto=http;apistatus=disabled', m.http.groups.disabled));
        if (m.http.t.down)           items.push(_sdExc('d-bad',  'ph-fill ph-warning-octagon',     m.http.t.down,           t('backends unreachable'), 'tab=services;proto=http;apistatus=unreachable', m.http.groups.down));
        if (m.http.t.degraded)       items.push(_sdExc('d-warn', 'ph-fill ph-warning-diamond',     m.http.t.degraded,       t('backends degraded'),    'tab=services;proto=http;apistatus=degraded', m.http.groups.degraded));
        if (m.http.t.warning)        items.push(_sdExc('d-warn', 'ph-fill ph-warning',             m.http.t.warning,        t('router warnings'),      'tab=services;proto=http;apistatus=warning',  m.http.groups.warning));
        ...
        if (m.service.t.disabled)    items.push(_sdExc('d-bad',  'ph-fill ph-x-circle',            m.service.t.disabled,    t('services disabled'),    'tab=live;svcstatus=error',                   m.service.groups.disabled));
        if (m.service.t.down)        items.push(_sdExc('d-bad',  'ph-fill ph-arrow-fat-line-down', m.service.t.down,        t('services down'),        'tab=live;svcstatus=error',                   m.service.groups.down));
        if (m.service.t.degraded)    items.push(_sdExc('d-warn', 'ph-fill ph-warning-diamond',     m.service.t.degraded,    t('services degraded'),    'tab=live;svcstatus=warning',                 m.service.groups.degraded));
        if (m.service.t.warning)     items.push(_sdExc('d-warn', 'ph-fill ph-warning',             m.service.t.warning,     t('service warnings'),     'tab=live;svcstatus=warning',                 m.service.groups.warning));
        if (m.middleware.t.disabled) items.push(_sdExc('d-bad',  'ph-fill ph-x-circle',            m.middleware.t.disabled, t('middlewares disabled'), 'tab=middlewares',                            m.middleware.groups.disabled));
